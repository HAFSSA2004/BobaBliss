...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (isHydrated) {
      localStorage.setItem("cartFavorites", JSON.stringify(favorites))
    }
  ...
  const toggleFavorite = (item: CartItem) => {
    setFavorites((prevFavorites) => {
      ...
      if (existingIndex > -1) {
        return prevFavorites.filter((_, i) => i !== existingIndex)
      }
  ...
  return (
    <CartContext.Provider value={{ items, favorites, addItem, removeItem, updateQuantity, toggleFavorite, clearCart }}>
      {children}
